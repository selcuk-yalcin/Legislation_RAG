...
so that web results become searchable alongside local documents.
"""
...
# Separate collection for web-sourced documents
WEB_COLLECTION_NAME = os.getenv("WEB_COLLECTION_NAME", "web_documents")


class WebVectorStore:
    """
    Vectorizes web-fetched document chunks with Voyage-law-2
    and stores them in a dedicated MongoDB collection.
    Also checks for duplicates to avoid re-processing the same URL.
    """
    ...
    def __init__(self):
        ...
        print(f"✅ WebVectorStore initialized (collection: {WEB_COLLECTION_NAME})")
    ...
    def url_already_stored(self, url: str) -> bool:
        ...
        return self.collection.count_documents({"metadata.source_url": url}) > 0

    def store_chunks(self, chunks: List[Dict]) -> int:
        """
        Vectorize and store a list of chunks into MongoDB.
